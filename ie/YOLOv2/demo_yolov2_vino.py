...
def ParseYOLOV2Output(
    ...
    print("ParseYOLOV2Output","resized",resized_im_h,resized_im_w," orig",original_im_h,original_im_w,threshold)
    num = 5
    coords = 4
    classes=20
    side_h = 13
...
def overlay_objects(image, objects):
    for obj in objects:
        ...
        #cv2.putText(image, class_names[label], p1, cv::FONT_HERSHEY_TRIPLEX, 0.4, cv::Scalar(255, 0, 0), 0.2);
...
for f in files:
    ...
    in_frame = in_frame[np.newaxis,:,:,:]        # Add new axis as a batch dimension HWC NHWC
    ...
    if exec_net.requests[0].wait(-1) == 0:
        ...
        print("frame_org.shape",frame_org.shape)
    ...
    #show result image
    cv2.imshow('YOLOv2_demo',frame_org)
    while True:
        key=cv2.waitKey(10)
        if key!=-1:
