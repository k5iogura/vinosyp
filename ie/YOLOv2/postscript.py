...
def softmax(x,axis=None):
    exp_x = np.exp(x)
    y = exp_x / np.sum(np.exp(x), axis=axis, keepdims=True)
    return y
...
def draw_boxes(im_org,sorted_boxes,classes,block_x,block_y,colors):
  ...
  for by in range(block_y):
    for bx in range(block_x):
      for j in range(bb_num):
        ...
        #probs = softmax(ans[by,bx,j,5:(5+class_num)])[0]   #PPAP
        probs = softmax(ans[by,bx,j,5:(5+class_num)])
...
def postscript(im_org,ans):

  # from yolov2-voc.cfg, we removed PPAP infomations
  img_x=416
  img_y=416
  block_x=13
  block_y=13
  ...
  biases =[[1.3221 ,1.73145],[3.19275,4.00944],
           ...
           [11.2364,10.0071]]
  #biases =[[ 1.08,1.19],[3.42,4.41],[6.63,11.38],[9.42,5.11],[16.62,10.52]]    #PPAP
  # th = 0.85
  # nms = 0.5
  th = 0.6
  nms = 0.05
  ...
  # get CNN output
  #ans = go_cnn(model,im_org, img_x,img_y,block_x,block_y,bb_num,class_num) #PPAP
  ...
  # Draw boxes
  im_marked = draw_boxes(im_org, sorted_boxes, classes, block_x, block_y, colors)
