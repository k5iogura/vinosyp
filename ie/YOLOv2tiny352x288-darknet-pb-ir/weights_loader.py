...
def load_conv_layer_bn(name,loaded_weights,shape,offset):
    ...
    for i in range(n_output_channels):
        ...
        assert math.isnan(gammas[i]) is False,"idx:{} gamma:{}".format(i,gammas[i])
        assert math.isnan(var[i]) is False and var[i]>=0,   "idx:{} var:{}".format(i,var[i])
        assert math.isnan(biases[i]) is False,"idx:{} bias:{}".format(i,biases[i])
        assert math.isnan(scale) is False,"idx:{} scale:{}".format(i,scale)
...
def load(sess,weights_path,ckpt_folder_path,saver):
    ...
    major_version  = 4  # for yolov2tiny_416x416
    major_version  = 5  # for yolov2tiny_352x288
    loaded_weights = loaded_weights[major_version:]
